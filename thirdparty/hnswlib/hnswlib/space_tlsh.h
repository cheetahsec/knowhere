...
static void reverse_bytes(unsigned char *bytes, int size)
{
    ...
    int end = size - 1;
    while (start < end) {
        unsigned char temp = bytes[start];
...
static int tlsh_diff(const char *hash_l, const char *hash_r)
{
    ...
    memcpy((void*)&lsh_bin_l, hash_l, sizeof(LshBinStruct));
    tlsh_impl_from_tlsh_str(&lsh_bin_l, hash_l);
    LshBinStruct lsh_bin_r;
    memcpy((void*)&lsh_bin_r, hash_r, sizeof(LshBinStruct));
...
TLSHDistance(const void *pVect1v, const void *pVect2v, const void *qty_ptr) {
    ...
    //     printf("%02x", ((const uint8_t*)pVect1v)[i]);
    // }
    // printf("\n");
    // for (int i = 0; i < size; i++) {
    //     printf("%02x", ((const uint8_t*)pVect2v)[i]);
    // }
    // printf("\n");

    int diff = tlsh_diff((const char*)pVect1v, (const char*)pVect2v);
    // printf("diff:%d\n", diff);
    return diff;
}
